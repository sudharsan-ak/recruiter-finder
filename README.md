...
### Core Scan
- Click **Find Recruiters** from any LinkedIn job page â€” the extension reads the company, navigates to their People tab in a background tab, scrolls through results, and returns a filtered list of recruiters
...
### Auto-Scan
- Toggle **Auto-scan as I browse** â€” the extension quietly scans each new company you view on LinkedIn without any manual action
- Only scans companies not already in cache

### Scan Queue
- While a scan is running, you can queue additional companies with **Add to Queue**
- The queue persists across panel close/reopen â€” if you close the panel mid-scan it picks up where it left off when you reopen it
...
- Click **ðŸ‘ Show** to open a picker modal â€” select which ones to add, leave others pending
- Dismissing (âœ•) allows the notification to reappear the next time you search or scroll â€” it removes those URLs from the seen set so they can be re-detected
...
- ðŸ”µ **Technical Recruiters** â€” technical / tech / sourcing in title
- ðŸŸ£ **Senior / Head of Recruiting** â€” senior / head / director / VP / lead
- ðŸŸ¡ **Coordinators**
- ðŸ©· **Talent Acquisition**
- ðŸŸ¢ **General Recruiters**
...
5. The extension icon appears in your toolbar â€” click it or press `Alt+R` (`Cmd+Shift+R` on Mac) to open the side panel
...
3. Click **ðŸš€ Find Recruiters** â€” the extension scans the company automatically
4. Results appear grouped by recruiter type
5. Use checkboxes + **Copy Selected Links** to copy profiles you want to reach out to

**For deeper searches:** Navigate to a company's People tab on LinkedIn and scroll/search â€” the observer will surface additional recruiters not found in the automated scan.
...
6. Saves results to `chrome.storage.local` â€” survives browser restarts
